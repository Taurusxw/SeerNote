...
- 搜索标题/正文/分类、收藏分组与持久化手工顺序。
- Note 拖放排序、`Alt+↑/↓` 等价动作、过滤视图隐藏槽位、跨收藏组拒绝和选择保持。
- `{{变量}}` 去重、替换、取消和空变量处理。
- 首次保存、原子替换、备份、主文件损坏恢复，以及 schema 1/2 到 schema 3 的首次显示顺序兼容。
- 相同 ID 或不支持 schema 的候选被拒绝。
- 回收站还原和永久删除领域行为。
- 单实例第二启动退出。
- CLI schema、创建/检索/更新/软删除/回收站/还原、目标组顶部顺序、结构化错误、退出码、stdin 正文与工作区锁冲突。
...
- Note 同组拖放插入线、跨收藏组拒绝、长列表边缘自动滚动与键盘排序。
- 150% DPI 或等价缩放。
